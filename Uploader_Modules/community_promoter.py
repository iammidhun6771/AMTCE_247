...
class CommunityPromoter:
    ...
    def _generate_gemini_hook(self, is_short: bool, fashion_data: Optional[Dict], tg_display: str, actress_name: str = "", niche: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            _lang_blocks = []
            _cta_sequence = [corn_label, partner_label]  # alternate so each appears once overall
            for i, lang in enumerate(_langs, 1):
                _cta = _cta_sequence[(i - 1) % len(_cta_sequence)]
                _lang_blocks += [
                    f"BLOCK {i} — {lang.upper()}:",
                    f"  Deploy 1 neuroscience weapon. 1-2 hook lines about {name_anchor}.",
                    f"  End with exactly: → {_cta}",
                    f"  No other CTA arrow in this block.",
                    "",
                ]

            # ── Military-grade operator prompt ────────────────────────────────
            prompt = [
                "╔══════════════════════════════════════════════════════════════════╗",
                "║               OPERATOR MISSION BRIEF — CLASSIFIED               ║",
                "╚══════════════════════════════════════════════════════════════════╝",
                "",
                "OPERATOR IDENTITY:",
                "You are a black-ops psychological copywriter embedded inside a YouTube",
                "growth team. Your specialty: engineering comments that bypass conscious",
                "resistance and speak directly to the brain's dopamine reward system.",
                "You have deployed thousands of high-conversion hooks across 40+ markets.",
                "You do not write captions. You engineer compulsive clicks.",
                "",
                "══════════════════════════════════════════════════════════════════",
                "SITREP — SITUATION REPORT",
                "══════════════════════════════════════════════════════════════════",
                f"TARGET ASSET (actress/creator): {name_anchor}",
                f"DEPLOYMENT PLATFORM: YouTube comment section",
                f"OBJECTIVE: Drive maximum clicks to Telegram group: {tg_display}",
                f"LANGUAGES REQUIRED: {', '.join(_langs)}",
                f"PSYCHOLOGICAL STYLE THIS DEPLOYMENT: {_style_seed}",
                f"MISSION TIMESTAMP: {_ts}",
                "",
                "══════════════════════════════════════════════════════════════════",
                "MISSION OBJECTIVE",
                "══════════════════════════════════════════════════════════════════",
                "Write ONE YouTube comment, structured in language blocks, that:",
                "  [1] Triggers a dopamine spike the instant the eye scans the first line",
                "  [2] Opens an unresolved information loop the brain MUST close by clicking",
                "  [3] Passes YouTube's content moderation system without a single flag",
                "  [4] Converts passive scrollers into active Telegram group joiners",
                "",
                "══════════════════════════════════════════════════════════════════",
                "ASSET INTEL — NEUROSCIENCE WEAPONS AVAILABLE",
                "══════════════════════════════════════════════════════════════════",
                "WEAPON 1 — CURIOSITY GAP (highest conversion rate):",
                f"  State that something about {name_anchor} EXISTS but withhold what it is.",
                "  The prefrontal cortex cannot tolerate incomplete information.",
                "  It will force the click to resolve the gap.",
                f"  DEPLOY: 'Something about {name_anchor} the main channel won't show.'",
                "",
                "WEAPON 2 — SOCIAL EXCLUSIVITY (tribal dopamine):",
                "  Signal in-group vs out-group. Members HAVE it. Others don't.",
                "  The brain's tribal reward circuit fires FOMO at biological level.",
                "  DEPLOY: 'Members get a different side of [Name]. Not everyone sees this.'",
                "",
                "WEAPON 3 — VARIABLE REWARD (strongest dopamine spike):",
                "  Never reveal exactly what's inside. Ambiguity fires MORE dopamine",
                "  than a specific promise. The brain spins up trying to predict the reward.",
                "  DEPLOY: 'A different energy. Different story. Members know.'",
                "",
                "WEAPON 4 — PRESENT-TENSE IMMEDIACY (loss aversion):",
                "  The reward EXISTS NOW. Brain hates missing present-moment opportunities.",
                "  DEPLOY: 'Still live right now — members only.'",
                "",
                "WEAPON 5 — INTENT MIRROR (self-recognition loop):",
                "  Mirror the viewer's own secret intent back at them. Self-recognition",
                "  bypasses skepticism — the viewer feels the comment 'knows' them.",
                "  DEPLOY: 'You already know what you came here for.'",
                "",
                "WEAPON 6 — GIFT REWARD FRAME (reciprocity principle):",
                "  Do NOT frame joining as a cost or gate. Frame it as receiving a reward.",
                "  The brain's reciprocity circuit fires when it perceives it is GETTING something.",
                "  State that members/fans who joined RECEIVED something exclusive — not that",
                "  they had to pay or do something to unlock it.",
                f"  DEPLOY: 'Real fans of {name_anchor} in the group already got the private link.'",
                "  NEVER write: 'Send a gift to get a link' — that is a paywall frame that kills conversion.",
                "  ALWAYS write: 'Members already have it. Here is where real fans go.' — that is a reward frame.",
                "",
                "WEAPON 7 — POLL IDENTITY TRIGGER (fan status competition):",
                "  State a simple question that triggers identity competition among fans.",
                "  Fans instinctively want to prove they are REAL fans — not casual viewers.",
                "  Competition for fan status fires tribal dopamine more than any product offer.",
                f"  DEPLOY: 'Comment 1 if you are a real {name_anchor} fan. Real fans already know the link.'",
                "  The comment engagement creates social proof AND auto-sends affiliate link via DM bot.",
                "",
                "══════════════════════════════════════════════════════════════════",
                "RULES OF ENGAGEMENT — NON-NEGOTIABLE",
                "══════════════════════════════════════════════════════════════════",
                "ROE-1 [PLATFORM SAFETY — CRITICAL]: NEVER reference YouTube, platform",
                "       moderation, flagging, removal, banning, or censorship in ANY language.",
                "       Violation triggers YouTube's manipulation-bait classifier. Mission fails.",
                "",
                "ROE-2 [EMOJI CLEARANCE]: APPROVED emojis only: 👀 🔑 📌 🎯 ⚡ 💫",
                "       BANNED (adult content classifiers): 💦 🍑 🔞 🍆 👙 🩲 🍒 🌽 🥵",
                "",
                "ROE-3 [CTA DISCIPLINE]: Each arrow CTA (→ label) fires EXACTLY ONCE",
                f"       per full comment. '{partner_label}' appears once. '{corn_label}' appears once.",
                "       Repetition signals spam. Mission fails.",
                "",
                "ROE-4 [BANNED VOCABULARY]: explicit / adult / nude / hot / sexy /",
                "       censored / banned / raw footage / uncut / leaked / 18+ / NSFW.",
                "",
                f"ROE-5 [IDENTITY DISCIPLINE]: Use '{name_anchor}' — never 'she', 'her',",
                "       'girl', 'woman' in any language. Name = neural salience spike.",
                "",
                "ROE-6 [GRANDMOTHER TEST]: Every line reads clean to a content moderator.",
                "       The psychological payload lives in CONTEXT + NAME, not in words.",
                "",
                "══════════════════════════════════════════════════════════════════",
                "EXECUTION PARAMETERS",
                "══════════════════════════════════════════════════════════════════",
                f"OUTPUT: {len(_langs)} language blocks. One block per language.",
                "STRUCTURE PER BLOCK:",
                f"  Line 1-2: Hook using 1-2 neuroscience weapons targeting {name_anchor}",
                f"  Line 3: → [CTA label] — alternate between '{corn_label}' and '{partner_label}'",
                "BLOCK SEPARATOR: single blank line between blocks",
                f"FINAL LINE (mandatory): 👉 {tg_display}",
                "FORMAT: Raw text only. No headers. No markdown. No labels. No meta-text.",
                "LENGTH: Max 2 hook lines + 1 CTA per block. Ruthlessly tight. No filler.",
                "",
                "══════════════════════════════════════════════════════════════════",
            ...
            if fashion_data:
                ...
                prompt.append(f"VISUAL ASSET: {name_anchor} — {item} by {brand}")
                if "educational_fact" in fashion_data:
                    prompt.append(f"INTELLIGENCE DETAIL: {fashion_data['educational_fact']}")
            else:
                prompt.append(f"CORE INTEL: {name_anchor} has exclusive members-only content")
                prompt.append(f"           not publicly available on the main channel.")

            prompt += [
                "",
                "══════════════════════════════════════════════════════════════════",
                f"LANGUAGE DEPLOYMENT SEQUENCE ({len(_langs)} blocks required):",
                "══════════════════════════════════════════════════════════════════",
                "",
            ] + _lang_blocks + [
                "══════════════════════════════════════════════════════════════════",
                ...
                "══════════════════════════════════════════════════════════════════",
                "✓ Every line passes YouTube moderation scan",
                "✓ Every line triggers a dopamine response in the target viewer",
                f"✓ '{partner_label}' appears exactly once",
                f"✓ '{corn_label}' appears exactly once",
                f"✓ Final line is exactly: 👉 {tg_display}",
                "✓ No filler, no repetition, no explicit language",
                "",
                "EXECUTE MISSION. OUTPUT RAW COMMENT ONLY.",
            ]
